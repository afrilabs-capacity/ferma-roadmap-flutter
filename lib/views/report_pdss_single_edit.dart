...
class EditReportPdss extends StatefulWidget {

  final PdssReportData report;
  EditReportPdss({this.report});

  @override
  _EditReportPdssState createState() => _EditReportPdssState();
}

class _EditReportPdssState extends State<EditReportPdss> {
...
//    TextEditingController passwordController =TextEditingController(text: '1234567');
  ...
  Future<void> submit(BuildContext context,String submitType) async {
    ...
    if (form.validate() && selectsValidCounter==0) {
      ...
      //print("in submit box");
      var submit= await Provider.of<PdssReportProvider>(context,listen: false).submitEditedReport(reportId: widget.report.id.toString(),firstName: firstNameController.text,lastName: lastNameController.text,gender: genderValue,marriage: marriageValue, occupation: occupationValue,pod:podController.text, dateArrested: dateArrestedController.text,dateReleased: dateReleasedController.text, offence: offenceValue,councilAssigned: counselValue,duration: durationController.text);
      ...
      if(submit['success']){
        ...
        Navigator.push(context, CupertinoPageRoute(builder: (context) => ReportPdssSingle(report:widget.report)));
      }else{
        bottomSheetController.close();
        setState(() {
          //this.modalWindowOpen=false;
  ...
  void initState() {
    ...
    WidgetsBinding.instance
      ...
      await Provider.of<PdssReportProvider>(context,listen: false).fetchCounsels().then( (value)async{
        await Provider.of<PdssReportProvider>(context,listen: false).fetchReportSingle(widget.report.id.toString()).then((value){
          PdssReportData singleReport=Provider.of<PdssReportProvider>(context,listen: false).report;
          if(singleReport!=null)firstNameController.text=singleReport.firstName;
          if(singleReport!=null)lastNameController.text=singleReport.lastName;
          //if(singleReport!=null)bailStatusController.text=singleReport.bailStatus;

          setState(() {
            //if(singleReport!=null)ageValue=singleReport.age.toString();
            if(singleReport!=null)genderValue=singleReport.gender;
            if(singleReport!=null)occupationValue=singleReport.occupation;
            if(singleReport!=null)offenceValue=singleReport.offence;
            if(singleReport!=null)podController.text=singleReport.pod;
            if(singleReport!=null)dateArrestedController.text=singleReport.dateArrested;
            if(singleReport!=null)dateReleasedController.text=singleReport.dateReleased;
            if(singleReport!=null)counselValue=singleReport.councilAssigned;
            if(singleReport!=null)durationController.text=singleReport.duration;
            if(singleReport!=null)marriageValue=singleReport.marriage;
            //if(singleReport!=null)caseTypeValue=singleReport.caseType;
            //singleReport.caseType=="Civil" ? complaintsValue=singleReport.complaints: offenceValue=singleReport.offence;
//            if(singleReport!=null)grantedValue=singleReport.granted;
//            if(singleReport!=null)locationValue=singleReport.location['id'];
//            if(singleReport!=null)outcomeValue=singleReport.outcome;
                      ...
                      child: SingleChildScrollView(
                          child:  Column(
                            ...
                            children: [
                              ...
                              Divider(),
                              SizedBox(height: 25.0,),
                              Text("Editing(PDSS Report)",style: TextStyle(color: siteThemeColor,fontSize: 22.0),),
                              SizedBox(height: 25.0,),

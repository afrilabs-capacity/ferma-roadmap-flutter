...
import 'package:lac/views/Register_form.dart';
...
class _RegisterState extends State<Register> {
  ...
  Future sendEmail ({BuildContext context,Function snackBar}) async{
    ...
    var register= await Provider.of<AuthProvider>(context,listen: false).getRegToken(email:emailController.text);
    //print(register.body);
    if(register['success']){
  ...
  Future sendCode ({BuildContext context,Function snackBar}) async{
    PersistentBottomSheetController bottomSheetController =await showModalBottom(context);
    var register= await Provider.of<AuthProvider>(context,listen: false).validateRegToken(email:emailController.text,code: codeController.text);
    //print(register.body);
    if(register['success']){
      bottomSheetController.close();
      modalWindowOpen=!modalWindowOpen;
      Navigator.pushReplacement(context, CupertinoPageRoute(builder: (context) =>  RegisterForm(userEmail:emailController.text)));
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                          ...
                          children: <Widget>[
                            ...
                            !codeState? Text("Register",style: TextStyle(color: siteThemeColor,fontSize: 30.0),): SizedBox(),
                            SizedBox(height: 10,),

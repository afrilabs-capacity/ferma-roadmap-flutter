...
import 'dart:io';
import 'dart:convert';
import 'package:lac/views/report_pdss_single_edit.dart';

class ReportPdssSingle extends StatefulWidget {

  final PdssReportData report;
  ReportPdssSingle({this.report});

  @override
  _ReportPdssSingleState createState() => _ReportPdssSingleState();
}

class _ReportPdssSingleState extends State<ReportPdssSingle> {
...
//    TextEditingController passwordController =TextEditingController(text: '1234567');
  ...
  @override
  void initState() {
    Provider.of<PdssReportProvider>(context,listen: false).report=null;
            ...
            Consumer<PdssReportProvider>(
              builder:(context,data,child)=> GestureDetector(
                  onTap: ()=> Navigator.push(
                    context,
                    CupertinoPageRoute(builder: (context) => EditReportPdss(report: widget.report)),
                  ),
                  child:Container(
                      padding:EdgeInsets.symmetric(horizontal: 15.0,vertical: 5.0),
                      margin:EdgeInsets.all(3.0),
...
class ReportItem extends StatelessWidget {
  final PdssReportData reportData;
                  ...
                  child:  ListView(
                    children: <Widget>[
                      Row(crossAxisAlignment: CrossAxisAlignment.start,
                        children: <Widget>[
                          Expanded(child: Consumer<PdssReportProvider>(
                            builder:(context,data,child)=> Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              //mainAxisAlignment: MainAxisAlignment.start,
                              children: <Widget>[
                                ...
                                ProfileCard(header: 'Marital Status',body: data.report.occupation),
                                ProfileCard(header: 'Occupation',body: data.report.occupation),
                                ProfileCard(header: 'Offence',body: data.report.offence),
                                ProfileCard(header: 'Place of Detention',body: data.report.pod),
                                ProfileCard(header: 'Date Arrested',body: data.report.dateArrested),
                                ProfileCard(header: 'Date Released',body: data.report.dateReleased),
                                ProfileCard(header: 'Counsel Assigned',body: data.report.councilAssigned),
                                ProfileCard(header: 'Duration',body: data.report.duration),
...
//  bool uploadTwoSuccess;
  ...
  showLoginModal(BuildContext context,String imageId, String reportId){
                            ...
                            child: Consumer<PdssReportProvider>(
                              ...
                              !data.apiCallModal ? Row(
                                ...
                                children: [
                                  ...
                                  RaisedButton(
                                      onPressed: ()async{
                                        var delete=  await Provider.of<PdssReportProvider>(context,listen: false).deleteReportImage(imageId: imageId, reportId: reportId);

                                        if(delete['success']){
                                          Navigator.pop(context);
                                          Provider.of<PdssReportProvider>(context, listen: false).fetchReportSingle(reportId);
...
//                          }),
  ...
  Future<void> _uploadOne() async{
    ...
    var  response = await  Provider.of<PdssReportProvider>(context, listen: false).uploadReportImage(image:base64Image,imageId:"1",reportId: widget.reportData.id.toString());

    if(response['success']){
      fileOne =null;
      Provider.of<PdssReportProvider>(context, listen: false).fetchReportSingle(widget.reportData.id.toString()).then((value) => setState(() {apiCallOne=false;}));
  ...
  Future<void> _uploadTwo()  async{
    ...
    var  response = await  Provider.of<PdssReportProvider>(context, listen: false).uploadReportImage(image:base64Image,imageId:"2",reportId: widget.reportData.id.toString());

    if(response['success']){
      fileTwo =null;
      Provider.of<PdssReportProvider>(context, listen: false).fetchReportSingle(widget.reportData.id.toString()).then((value) => setState(() {apiCallTwo=false;}));
                            ...
                            data.report.photoOne!=null ? Positioned(
                              ...
                              child: CircleAvatar(
                                ...
                                child: IconButton(
                                  ...
                                  onPressed:()=>showLoginModal(context,"1",data.report.id.toString()),
                                ),
                              ),
                            ...
                            data.report.photoTwo!=null ? Positioned(
                              ...
                              child: CircleAvatar(
                                ...
                                child: IconButton(
                                  ...
                                  onPressed:  ()=>showLoginModal(context,"2",data.report.id.toString()),
                                ),
                              ),

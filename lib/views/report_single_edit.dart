...
//    TextEditingController passwordController =TextEditingController(text: '1234567');
  ...
  Future<void> submit(BuildContext context,String submitType) async {
    ...
    if (form.validate() && selectsValidCounter==0) {
      ...
      print("in submit box");
      var submit= await Provider.of<ReportProvider>(context,listen: false).submitEditedReport(reportId: widget.report.id.toString(), firstName: firstNameController.text,lastName: lastNameController.text,gender: genderValue,age: ageValue,occupation: occupationValue,granted: grantedValue,caseType: caseTypeValue,offence: offenceValue,complaints: complaintsValue,location: locationValue,bailStatus: bailStatusController.text,outcome: outcomeValue);
      ...
      if(submit['success']){
        ...
        //print(submit['data']);
        Navigator.push(context, CupertinoPageRoute(builder: (context) => ReportSingle(report:widget.report)));
      }else{
        bottomSheetController.close();
        //modalWindowOpen=!modalWindowOpen;
  ...
  void initState() {
    ...
    WidgetsBinding.instance
      ...
      await Provider.of<ReportProvider>(context,listen: false).fetchReportSingle(widget.report.id.toString()).then((value){
       ReportData singleReport=Provider.of<ReportProvider>(context,listen: false).report;
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                          ...
                          children: <Widget>[
                            ...
                            Divider(),
                            SizedBox(height: 25.0,),
                            Text("Editing (Daily Report)",style: TextStyle(color: siteThemeColor,fontSize: 22.0),),
                            SizedBox(height: 25.0,),
                            TextFormField(
